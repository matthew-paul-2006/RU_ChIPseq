---
title: "ChIP-seq In Bioconductor <html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>"
...
# Working with raw ChIPseq sequences.
...
---
# Rbowtie2 ChIP-seq alignment

The **QuasR** package uses Bowtie aligner under the hood to map ur reads to the genome. We have seen this is a little slow and memory hungry. 
In latest Bioconductor releases we have a newer Bowtie version available to us the in the **Rbowtie2** package.
...
The **Rbowtie2** package requires us to first to create an index to align to.
...
We would need to make sure we delete any tempory files created (SAM and/or uncompressed FastQ) to avoid fillin up our harddrives. We can delete files in R usin the **unlink()** function.

```{r bsgxxxnoaaxssme, echo=TRUE,eval=FALSE}
unlink("ENCFF001NQP.sam")
unlink("Data/ENCFF001NQP.fq")
```
